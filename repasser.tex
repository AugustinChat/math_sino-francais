...
\usepackage{amsmath, amsthm, amssymb, bm, color, framed, graphicx, 
...
\section{集合}
  为了便于你的理解,我们先给出集合的定义,并且顺着这些定义讨论,随后会在\prettyref{myref:公理化}这里把这些概念全部公理化,以求得到更深刻的理解.
...
\section{集合的运算}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[scale=0.05]{Produit_Cartésien.png}
  ...
  现在,回头看看\prettyref{myref:确定性},尝试回答以下问题:\\
...
\section{无限集的可数性}
  ...
  并且\prettyref{myref:Z可数}告诉我们$\mathbb{Z}$是可数的，\prettyref{myref:Rn可数}告诉我们$\mathbb{Z}^2$是可数的。
  ...
  若$\mathbb{I}$是至多可数的，则根据\prettyref{myref:可数集可数并可数}可知$\mathbb{R}$是至多可数的,这显然矛盾。
  故$\mathbb{I}$是不可数的。
...
\section{代数结构 Structure Algébrique}
  ...
  现在,我希望你假装忘记掉曾经学习的这些运算,也就是\prettyref{myref:传统运算}开头我提到的那些以前的东西(特别是加法和乘法),以便我们从另一个角度重新出发学习它们.
...
\section{群 Groupe}
  ...
  这就是为什么\prettyref{myref:C_3群}介绍的群叫$C_3$群.在$C_3$群中,a和b都是其生成元.对于$n$阶循环群, 其阶数等于其生成元的阶数.
...
\section{相对开集/相对闭集}
  ...
  $$
  这个概念非常像\prettyref{myref:子空间拓扑}提到的子空间拓扑.
...
与讲义上的顺序不同，我打算直接定义$L^P$范数(又称为$P$范数)并研究它的性质，而不是通过需要的性质去定义三个范数。
  ...
  $$
  称$B$是以$q$为中兴半径$r$的开球。请对照\prettyref{myref:邻域},开球与邻域的区别，其实就是度量和范数的差异。
  \subsection{单位球与等距线}
...
\section{拓扑空间}
  ...
  例如，\prettyref{myref:离散度量}的离散度量可以诱导离散拓扑，此时邻域$N_\frac{1}{2}=\{X\}$就是开集。
...
\section{同胚 Homeomorphisme}
  ...
  \subsection{Exemple}
  \subsection{Remarque}
  请回顾我们在\prettyref{myref:同态}和\prettyref{myref:同构}里提到的同态和同构,它们有什么相似的地方?\\
...
\section{特征值与特征向量 Vecteurs Propres et Valeurs Propres}
  ...
  \subsubsection{Remarque}
  还记得\prettyref{myref:特征}介绍的“特征”的概念吗?想一下我们为什么要管这里的$\lambda$叫“特征”值.
...
\section{相似矩阵 Matrice semblable}
  ...
  \subsubsection{Remarque}
  嘿!还记得\prettyref{myref:共轭关系}提到的共轭关系吗?这就是共轭关系在矩阵里的应用!
...
\section{实对称矩阵 Matrice symétrique réelle}
  ...
  \begin{flalign*}
    \begin{aligned}
      ...
      & \Rightarrow \lambda^t\bar{x}x=A^t\bar{x}x=\overline{A^t\bar{x}x} =A^tx\bar{x}=\bar{\lambda}^tx\bar{x}\\
  ...
  \begin{flalign*}
    \begin{aligned}%怎么输入左上角标？这样打起来很不好看啊
      ...
      & \neg(^tab=0)\\
      & \Rightarrow \frac{\lambda_b}{\lambda_a}=\frac{^tba}{^tab}=\frac{^t(^tba)}{^tab}=1\\
